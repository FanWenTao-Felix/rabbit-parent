package ink.ckx.rabbit.task.parser;
...
import ink.ckx.rabbit.task.autoconfigure.JobZookeeperProperties;
import ink.ckx.rabbit.task.enums.ElasticJobTypeEnum;
    ...
    public void onApplicationEvent(ApplicationReadyEvent event) {
        ...
        try {
            ...
            for (Iterator<?> it = beanMap.values().iterator(); it.hasNext(); ) {
                ...
                //	把当当网的 es-job 的相关 configuration 先实例化出来
                ...
                //	1. 添加 bean 构造参数，相当于添加自己的真实的任务实现类
                if (!ElasticJobTypeEnum.SCRIPT.getType().equals(jobTypeName)) {
                ...
                // 	接下来就是把 factory 也就是 SpringJobScheduler 注入到 Spring 容器中
